  ...
  return (
    ...
    >
      ...
      <div className="mb-3 space-y-1">
        ...
        <div className={`p-2.5 border-[1.5pt] border-slate-950 border-t-0 ${textAlign} space-y-2`}>
          <div className="flex items-start gap-2">
            <span className="font-black text-[10px] uppercase tracking-tighter bg-indigo-600 text-white print:bg-slate-200 print:text-black px-1.5 leading-tight rounded-sm">TITRE :</span>
            <span className="font-black text-[11px] uppercase underline decoration-2 underline-offset-2 leading-tight">{renderBilingualText(sheet.title)}</span>
          </div>
          
          <div className="text-[9px] leading-snug">
            <span className="font-black uppercase text-indigo-700 print:text-black">Compétence de base :</span> 
            <span className="ml-1 italic font-medium">{renderBilingualText(sheet.competence)}</span>
          </div>

          <div className="bg-slate-100/50 print:bg-slate-50 p-2 border-l-4 border-indigo-600 print:border-slate-950">
            <span className="font-black uppercase text-[9px]">Objectif Spécifique (OS) :</span>
            <p className="font-bold text-[9.5px] leading-tight italic text-slate-800">{renderBilingualText(sheet.specificObjective)}</p>
          </div>
        </div>
      </div>

      {/* Intrants - Horizontal Row */}
      <div className={`grid grid-cols-4 gap-0 text-[8.5px] border-[1.5pt] border-slate-950 border-t-0 mb-3 bg-white ${isArabic ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`mb-3 border-[1.5pt] border-slate-950 p-3 bg-indigo-50/20 print:bg-transparent ${textAlign} relative overflow-hidden`}>
        <div className="flex items-center mb-1.5 border-b border-slate-300 pb-1">
          ...
          <span className="font-black uppercase text-[10px] tracking-widest text-indigo-900 print:text-black">Trace Écrite (Résumé de la leçon)</span>
        </div>
        <div className="text-[10px] md:text-[10.5px] leading-relaxed font-medium whitespace-pre-wrap italic text-slate-800">
          {renderBilingualText(sheet.contentSummary)}
        </div>
      </div>
        ...
        <table className="w-full border-collapse border-[1.5pt] border-slate-950 table-fixed">
          <thead>
            <tr className="bg-slate-900 text-white print:bg-slate-100 print:text-black font-black uppercase text-[7.5px] leading-tight">
              <th className="py-1.5 px-1 w-[12%] border-r-[1.5pt] border-slate-950 text-center">Étapes</th>
              <th className="py-1.5 px-1 w-[15%] border-r-[1.5pt] border-slate-950 text-center">Objectifs</th>
              <th className="py-1.5 px-1.5 w-[36.5%] border-r-[1.5pt] border-slate-950 text-left">Activités du Maitre</th>
              <th className="py-1.5 px-1.5 w-[36.5%] text-left">Activités des Élèves</th>
            </tr>
          </thead>
